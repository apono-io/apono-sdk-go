/*
Apono

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package apono

import (
	"encoding/json"
)

// checks if the AccessFlowTriggerV1Timeframe type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AccessFlowTriggerV1Timeframe{}

// AccessFlowTriggerV1Timeframe struct for AccessFlowTriggerV1Timeframe
type AccessFlowTriggerV1Timeframe struct {
	StartOfDayTimeInSeconds int64       `json:"start_of_day_time_in_seconds"`
	EndOfDayTimeInSeconds   int64       `json:"end_of_day_time_in_seconds"`
	DaysInWeek              []DayOfWeek `json:"days_in_week"`
	TimeZone                string      `json:"time_zone"`
}

// NewAccessFlowTriggerV1Timeframe instantiates a new AccessFlowTriggerV1Timeframe object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAccessFlowTriggerV1Timeframe(startOfDayTimeInSeconds int64, endOfDayTimeInSeconds int64, daysInWeek []DayOfWeek, timeZone string) *AccessFlowTriggerV1Timeframe {
	this := AccessFlowTriggerV1Timeframe{}
	this.StartOfDayTimeInSeconds = startOfDayTimeInSeconds
	this.EndOfDayTimeInSeconds = endOfDayTimeInSeconds
	this.DaysInWeek = daysInWeek
	this.TimeZone = timeZone
	return &this
}

// NewAccessFlowTriggerV1TimeframeWithDefaults instantiates a new AccessFlowTriggerV1Timeframe object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAccessFlowTriggerV1TimeframeWithDefaults() *AccessFlowTriggerV1Timeframe {
	this := AccessFlowTriggerV1Timeframe{}
	return &this
}

// GetStartOfDayTimeInSeconds returns the StartOfDayTimeInSeconds field value
func (o *AccessFlowTriggerV1Timeframe) GetStartOfDayTimeInSeconds() int64 {
	if o == nil {
		var ret int64
		return ret
	}

	return o.StartOfDayTimeInSeconds
}

// GetStartOfDayTimeInSecondsOk returns a tuple with the StartOfDayTimeInSeconds field value
// and a boolean to check if the value has been set.
func (o *AccessFlowTriggerV1Timeframe) GetStartOfDayTimeInSecondsOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.StartOfDayTimeInSeconds, true
}

// SetStartOfDayTimeInSeconds sets field value
func (o *AccessFlowTriggerV1Timeframe) SetStartOfDayTimeInSeconds(v int64) {
	o.StartOfDayTimeInSeconds = v
}

// GetEndOfDayTimeInSeconds returns the EndOfDayTimeInSeconds field value
func (o *AccessFlowTriggerV1Timeframe) GetEndOfDayTimeInSeconds() int64 {
	if o == nil {
		var ret int64
		return ret
	}

	return o.EndOfDayTimeInSeconds
}

// GetEndOfDayTimeInSecondsOk returns a tuple with the EndOfDayTimeInSeconds field value
// and a boolean to check if the value has been set.
func (o *AccessFlowTriggerV1Timeframe) GetEndOfDayTimeInSecondsOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.EndOfDayTimeInSeconds, true
}

// SetEndOfDayTimeInSeconds sets field value
func (o *AccessFlowTriggerV1Timeframe) SetEndOfDayTimeInSeconds(v int64) {
	o.EndOfDayTimeInSeconds = v
}

// GetDaysInWeek returns the DaysInWeek field value
func (o *AccessFlowTriggerV1Timeframe) GetDaysInWeek() []DayOfWeek {
	if o == nil {
		var ret []DayOfWeek
		return ret
	}

	return o.DaysInWeek
}

// GetDaysInWeekOk returns a tuple with the DaysInWeek field value
// and a boolean to check if the value has been set.
func (o *AccessFlowTriggerV1Timeframe) GetDaysInWeekOk() ([]DayOfWeek, bool) {
	if o == nil {
		return nil, false
	}
	return o.DaysInWeek, true
}

// SetDaysInWeek sets field value
func (o *AccessFlowTriggerV1Timeframe) SetDaysInWeek(v []DayOfWeek) {
	o.DaysInWeek = v
}

// GetTimeZone returns the TimeZone field value
func (o *AccessFlowTriggerV1Timeframe) GetTimeZone() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.TimeZone
}

// GetTimeZoneOk returns a tuple with the TimeZone field value
// and a boolean to check if the value has been set.
func (o *AccessFlowTriggerV1Timeframe) GetTimeZoneOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.TimeZone, true
}

// SetTimeZone sets field value
func (o *AccessFlowTriggerV1Timeframe) SetTimeZone(v string) {
	o.TimeZone = v
}

func (o AccessFlowTriggerV1Timeframe) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AccessFlowTriggerV1Timeframe) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["start_of_day_time_in_seconds"] = o.StartOfDayTimeInSeconds
	toSerialize["end_of_day_time_in_seconds"] = o.EndOfDayTimeInSeconds
	toSerialize["days_in_week"] = o.DaysInWeek
	toSerialize["time_zone"] = o.TimeZone
	return toSerialize, nil
}

type NullableAccessFlowTriggerV1Timeframe struct {
	value *AccessFlowTriggerV1Timeframe
	isSet bool
}

func (v NullableAccessFlowTriggerV1Timeframe) Get() *AccessFlowTriggerV1Timeframe {
	return v.value
}

func (v *NullableAccessFlowTriggerV1Timeframe) Set(val *AccessFlowTriggerV1Timeframe) {
	v.value = val
	v.isSet = true
}

func (v NullableAccessFlowTriggerV1Timeframe) IsSet() bool {
	return v.isSet
}

func (v *NullableAccessFlowTriggerV1Timeframe) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAccessFlowTriggerV1Timeframe(val *AccessFlowTriggerV1Timeframe) *NullableAccessFlowTriggerV1Timeframe {
	return &NullableAccessFlowTriggerV1Timeframe{value: val, isSet: true}
}

func (v NullableAccessFlowTriggerV1Timeframe) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAccessFlowTriggerV1Timeframe) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
